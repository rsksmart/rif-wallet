...
import {SLIDER_HEIGHT, SLIDER_WIDTH, WINDOW_WIDTH} from "../../slides/Dimensions";
...
> = ({ navigation, generateMnemonic }) => {
  ...
  const word = ({ number, text }: { number: number, text:string }) =>(<View style={styles.wordContainer}>
  ...
  const renderItem = ({ item }: { item: number }) =>{
    ...
    <View style={styles.slideContainer}>
      {word({number: 1 + wordIndex,text:mnemonicArray[1 + wordIndex -1]})}
      {word({number: 2 + wordIndex,text:mnemonicArray[2 + wordIndex -1]})}
      {word({number: 3 + wordIndex,text:mnemonicArray[3 + wordIndex -1]})}
    ...
    <>
      <ScrollView style={styles.parent}>
        ...
        <PaginationNavigator
            ...
            onComplete={() => navigation.navigate('ConfirmNewMasterKey', { mnemonic })}
            title="confirm"
