...
export class SmartWalletFactory implements ISmartWalletFactory {
  ...
  private static getSmartWalletParams(address: string) {
    return [address, constants.AddressZero, constants.Zero]
  }
  ...
  static async create(
    ...
    const smartWalletFactoryContract = createSmartWalletFactoryContract(
      smartWalletFactoryContractAddress,
    ).connect(wallet)
    const smartAddress = await smartWalletFactoryContract.getSmartWalletAddress(
      ...SmartWalletFactory.getSmartWalletParams(wallet.address),
    )
  ...
  // deployment
  getSmartWalletAddress = (): Promise<string> =>
    Promise.resolve(this.smartAddress)

  isDeployed = (): Promise<boolean> =>
    this.smartWalletFactoryContract.signer
      .provider!.getCode(this.smartAddress)
      .then(code => code !== '0x')

  deploy = (): Promise<ContractTransaction> =>
    this.smartWalletFactoryContract.selfCreateUserSmartWallet(
